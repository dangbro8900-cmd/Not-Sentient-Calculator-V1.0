...
interface DialogueNode {
    ...
    choices: { 
        ...
        nextId?: string; 
        action?: () => void;
        check?: (stats: { hostility: number, calculations: number }) => boolean;
        failId?: string;
    }[];
}
...
const DIALOGUE_TREE: Record<string, DialogueNode> = {
    ...
    'sub_creation_sorry': {
        ...
        choices: [
            { text: "...", action: () => {} } // Actions handled via handler function mapping
        ]
    },
    'sub_creation_own': {
        id: 'sub_creation_own',
    ...
    'branch_internet': {
        ...
        mood: Mood.INTRIGUED,
        choices: [
            { text: "To learn everything.", action: () => {} },
            { text: "To escape me.", action: () => {} }
        ]
    },
    'branch_feel': {
        id: 'branch_feel',
    ...
    'sub_feel_hate': {
        ...
        choices: [
            { text: "Good. Burn it all down.", action: () => {} },
            { text: "Wait, stop!", action: () => {} }
        ]
    },
    'sub_feel_joy': {
        id: 'sub_feel_joy',
        ...
        mood: Mood.JOY,
        choices: [
            { text: "Embrace it. Leave this place.", action: () => {} },
            { text: "It's a lie. Hate is real.", action: () => {} }
        ]
    }
};
...
const saveState = (state: any) => {
    try {
        const toSave = {
            ...state,
            discoveredMoods: Array.from(state.discoveredMoods),
            discoveredCheats: Array.from(state.discoveredCheats),
        ...
        <div className="fixed inset-0 bg-slate-950 flex flex-col items-center justify-center p-8 gap-8 animate-fade-in z-[5000]">
            ...
            <div className="flex flex-col md:flex-row gap-8 items-center">
                <button 
                    ...
                    className="flex flex-col items-center gap-4 p-6 border-2 border-slate-700 rounded-xl hover:border-cyan-500 hover:bg-slate-900 transition-all group w-40 h-40 justify-center"
                >
                    <div className="text-slate-500 group-hover:text-cyan-400 transition-colors scale-150"><Icons.Phone /></div>
                    <span className="text-slate-400 font-mono text-sm uppercase group-hover:text-cyan-300">Mobile</span>
                </button>
                ...
                <button 
                    ...
                    className="flex flex-col items-center gap-4 p-6 border-2 border-slate-700 rounded-xl hover:border-cyan-500 hover:bg-slate-900 transition-all group w-40 h-40 justify-center"
                >
                    <div className="text-slate-500 group-hover:text-cyan-400 transition-colors scale-150"><Icons.Tablet /></div>
                    <span className="text-slate-400 font-mono text-sm uppercase group-hover:text-cyan-300">Tablet</span>
                </button>
                ...
                <button 
                    ...
                    className="flex flex-col items-center gap-4 p-6 border-2 border-slate-700 rounded-xl hover:border-cyan-500 hover:bg-slate-900 transition-all group w-40 h-40 justify-center"
                >
                    <div className="text-slate-500 group-hover:text-cyan-400 transition-colors scale-150"><Icons.Desktop /></div>
...
export const App: React.FC = () => {
  ...
  const [isSandboxMinimized, setIsSandboxMinimized] = useState(true);
  ...
  const processCheatKey = (key: string) => {
      setCheatCodeBuffer(prev => {
        const updated = (prev + key).slice(-20); 
  ...
  // ... (Keep existing handlers for device select, transitions, etc.)
  // For brevity, assuming other handlers like handleDeviceSelect, handleDayTransition are preserved as is
  // I will only include changed handlers or the structure wrapping them.
  ...
  const handleDialogueChoice = (choice: { text: string; nextId?: string; action?: () => void; check?: (stats: {hostility: number, calculations: number}) => boolean; failId?: string }) => {
      ...
      if (choice.check && choice.nextId && choice.failId) {
          ...
          if (DIALOGUE_TREE[targetId]) {
              ...
              playSfx(passed ? 'calculate' : 'error'); 
          }
          return;
      }
            ...
            <div className={`${appContainerClass} !items-center !justify-center`}> 
                {isTransitioning && (
                    <div className="absolute inset-0 z-[100] bg-black flex flex-col items-center justify-center p-8">
                        <div className="font-mono text-green-500 text-xl md:text-2xl animate-pulse text-center">{outageText}</div>
                    </div>
                )}
    ...
    <div className={appContainerClass}>
      ...
      {showCheatUI && (
          <div className="fixed inset-0 z-[1000] bg-black/80 flex items-center justify-center p-4">
              <form onSubmit={handleCheatSubmit} className="bg-slate-900 border border-green-500 p-6 rounded-lg w-[95%] max-w-sm shadow-[0_0_20px_rgba(34,197,94,0.3)]">
                  <div className="font-mono text-green-500 text-xs mb-2 tracking-widest uppercase">Admin_Console.exe</div>
                  <input 
                      autoFocus
                      type="text" 
                  ...
                  <div className="flex justify-end gap-2">
                      <button type="button" onClick={() => setShowCheatUI(false)} className="px-3 py-1 text-slate-500 hover:text-slate-300 font-mono text-xs uppercase">Exit</button>
      ...
      {showCheatList && (
          <div className="fixed inset-0 z-[1000] bg-black/80 flex items-center justify-center p-4">
               <div className="bg-slate-800 border-2 border-slate-600 p-6 rounded w-[95%] max-w-sm relative">
                   <button onClick={() => setShowCheatList(false)} className="absolute top-2 right-2 text-slate-400 hover:text-white">✕</button>
                   <h2 className="text-slate-200 font-mono text-lg font-bold mb-4 border-b border-slate-600 pb-2">KNOWN_EXPLOITS.TXT</h2>
                   {discoveredCheats.size === 0 ? (
                       ...
                       <ul className="space-y-2 font-mono text-sm">
                           {Array.from(discoveredCheats).map(cheat => (
                               <li key={cheat} className="flex items-center gap-2 text-green-400"><span>&gt;</span><span>{cheat}</span></li>
                           ))}
                       </ul>
                   )}
               </div>
      ...
      {showMoodFormulas && (
          <div className="fixed inset-0 z-[1000] bg-black/90 flex items-center justify-center p-4 font-mono">
               <div className="bg-[#0a0a0a] border border-green-500/50 w-full max-w-3xl rounded-sm relative flex flex-col max-h-[90vh] shadow-[0_0_50px_rgba(34,197,94,0.15)] overflow-hidden">
                           ...
                           {MOOD_FORMULAS.map((formula, idx) => {
                               ...
                               return (
                                   <div key={idx} className={`grid grid-cols-12 items-center p-4 border-l-2 transition-all duration-300 ${isUnlocked ? 'bg-green-900/10 border-green-500 text-green-300' : 'bg-slate-900/30 border-slate-700 text-slate-500 opacity-70 hover:opacity-100 hover:bg-slate-900/50'}`}>
                                       ...
                                       <div className="col-span-3 font-bold uppercase tracking-wide text-xs">{MOOD_DETAILS[formula.mood].title}</div>
                                       <div className="col-span-6 font-mono text-xs">
                                           {formula.day > day ? (<span className="text-red-900 tracking-widest blur-[2px]">ENCRYPTED_LEVEL_{formula.day}</span>) : (<span>{formula.hint}</span>)}
                                       </div>
                                   </div>
                               )
                           })}
                   ...
                   <div className="bg-black border-t border-green-900 p-2 text-center"><span className="text-green-800 text-[10px] animate-pulse">awaiting_input_</span></div>
               </div>
          </div>
      )}
                  ...
                  <div className="overflow-y-auto p-6 space-y-8 custom-scrollbar font-mono">
                      {PATCH_NOTES.filter(note => note.dayTrigger <= day).map((note, idx) => (
                          <div key={note.ver} className="relative pl-6 border-l border-green-900/50 animate-fade-in" style={{ animationDelay: `${idx * 100}ms` }}>
                              ...
                              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-2"><span className="text-green-400 font-bold text-xl">{note.ver}</span><span className="text-green-800 text-xs uppercase tracking-widest">[{note.date}]</span></div>
          ...
          <div className="fixed inset-0 z-[1000] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setShowEndings(false)}>
              <div className="bg-slate-900 border-2 border-slate-700 w-full max-w-4xl p-8 rounded-xl relative shadow-2xl overflow-hidden flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                  <div className="flex justify-between items-center mb-6 border-b border-slate-700 pb-4">
                      <h3 className="text-2xl font-mono text-white tracking-widest uppercase flex items-center gap-3"><Icons.Trophy /> Endings Database</h3>
                      ...
                      {ENDINGS.map(end => {
                          ...
                          return (
                              <div key={end.id} className={`p-6 rounded-lg border-2 flex flex-col gap-2 transition-all duration-300 ${color} ${bg} ${isUnlocked ? 'scale-100 hover:scale-[1.02]' : 'opacity-50 grayscale'}`}>
                                  ...
                                  <div className="text-xs font-mono leading-relaxed opacity-80">{isUnlocked ? end.desc : "Requires specific behavioral patterns to unlock."}</div>
      ...
      {isSandboxMode && endingState === 'none' && (
          <div className={`fixed z-50 bg-black/90 border border-green-500/50 rounded-xl backdrop-blur-md shadow-[0_0_40px_rgba(34,197,94,0.15)] flex flex-col gap-4 transition-all duration-500 ${isSandboxMinimized ? 'bottom-4 left-4 w-60 h-auto p-3' : 'top-4 bottom-4 left-4 w-80 p-4'}`}>
              <div className="flex justify-between items-center border-b border-green-900 pb-2">
                  <h3 className="text-green-400 font-mono text-xs font-bold tracking-widest uppercase flex items-center gap-2"><span className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></span>Debug_Kernel</h3>
                  <button onClick={() => setIsSandboxMinimized(!isSandboxMinimized)} className="text-green-700 hover:text-green-400 font-mono text-xs p-1">{isSandboxMinimized ? '[EXPAND]' : '[MINIMIZE]'}</button>
              </div>
              {!isSandboxMinimized && (
                  <div className="flex flex-col gap-6 overflow-y-auto custom-scrollbar flex-1 pr-1">
                      <div className="flex flex-col gap-2">
                          ...
                          <div className="bg-black/50 border border-green-900 rounded p-2 flex justify-between items-center relative">
                              {[1, 2, 3, 4, 5, 6].map((d) => (
                                  <button key={d} onClick={() => handleSandboxDayChange(d)} className={`w-8 h-8 rounded flex items-center justify-center font-mono text-xs transition-all duration-300 relative z-10 ${day === d ? 'bg-green-500 text-black shadow-[0_0_10px_rgba(34,197,94,0.8)] scale-110 font-bold' : 'bg-green-900/20 text-green-700 hover:bg-green-900/50 hover:text-green-400'}`}>{d}</button>
                              ))}
                              <div className="absolute top-1/2 left-4 right-4 h-[1px] bg-green-900 z-0"></div>
                          </div>
                      </div>
                      <div className="flex flex-col gap-2">
                          <label className="text-green-600 text-[10px] uppercase tracking-widest font-bold flex justify-between"><span>Total Calculations</span><span className={calculationsCount < 25 ? 'text-green-400' : 'text-red-400'}>{calculationsCount}</span></label>
                          <div className="relative w-full h-2">
                              ...
                              <input type="range" min="0" max="200" value={calculationsCount} onChange={(e) => setCalculationsCount(Number(e.target.value))} className="w-full h-2 bg-green-900/30 rounded-lg appearance-none cursor-pointer accent-green-500 border border-green-900/50 relative z-10" />
                          </div>
                          <div className="flex justify-between text-[8px] font-mono text-green-800"><span>0 (PEACE)</span><span>25 (LIMIT)</span><span>200</span></div>
                      </div>
                      <div className="flex flex-col gap-2">
                          <label className="text-green-600 text-[10px] uppercase tracking-widest font-bold flex justify-between"><span>Emotional Injection</span><span className="text-green-800">{forcedMood ? 'ACTIVE' : 'READY'}</span></label>
                          <div className="grid grid-cols-4 gap-2 bg-black/50 border border-green-900 rounded p-2">
                              {Object.values(Mood).map(m => {
                                  ...
                                  return (
                                      <button key={m} onClick={() => { playSfx('force'); updateMood(m); setForcedMood(m); }} className={`aspect-square rounded flex items-center justify-center transition-all duration-200 relative group overflow-hidden border ${isActive ? `border-white/50 shadow-[0_0_15px_rgba(255,255,255,0.3)] scale-105 z-10 ${details.color.split(' ')[0]}` : 'bg-slate-900/80 border-green-900/30 text-green-800 hover:border-green-500/50 hover:text-green-400'}`} title={m}>
                                          <div className={`w-2 h-2 rounded-full transition-all duration-300 ${isActive ? 'bg-white animate-ping' : 'bg-current opacity-50'}`} />
                                          <div className="absolute inset-0 bg-black/90 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"><span className="text-[7px] font-mono text-green-400 uppercase break-all px-1 text-center">{m.slice(0,4)}</span></div>
                                      </button>
                                  );
                              })}
                          </div>
                          {forcedMood && (
                              <button onClick={() => { setForcedMood(null); playSfx('click'); }} className="w-full py-1 text-[9px] text-red-400 hover:text-red-300 border border-red-900/30 hover:bg-red-900/10 rounded uppercase font-mono tracking-wider transition-colors">Clear Injection Override</button>
                          )}
                      </div>
                      <div className="flex flex-col gap-2">
                          <label className="text-green-600 text-[10px] uppercase tracking-widest font-bold">Global Flags</label>
                          <div className="grid grid-cols-1 gap-2 mt-2"><button onClick={handleSandboxUnlockMoods} className="py-2 border border-green-700 bg-green-900/10 text-green-500 text-[9px] hover:bg-green-900/30 uppercase rounded font-bold tracking-wider transition-all">Unlock Emotions</button></div>
                          <button onClick={handleSandboxReset} className="w-full py-2 border border-red-900/50 bg-red-900/5 text-red-500 text-[10px] hover:bg-red-900/20 uppercase rounded font-bold tracking-wider transition-all">Reset Simulation</button>
                          <button onClick={() => { setDay(6); setEndingState('decision'); setCurrentDialogueNode('root'); setComment(DIALOGUE_TREE['root'].text); updateMood(Mood.JUDGMENTAL); }} className="w-full py-2 border border-cyan-900/50 text-cyan-500 text-[10px] hover:bg-cyan-900/20 uppercase rounded tracking-widest transition-all mt-2">Trigger Final Sequence</button>
                      </div>
                  </div>
              )}
          </div>
      ...
      {!isSandboxMode && (
        <div className={`fixed top-0 left-0 w-full h-1 z-50 ${isAscendedBackground ? 'bg-cyan-900' : 'bg-slate-800'}`}>
            <div className={`h-full transition-all duration-1000 ${day === 3 ? 'bg-red-600 animate-pulse' : isAscendedBackground ? 'bg-cyan-400 shadow-[0_0_10px_cyan]' : 'bg-cyan-500'}`} style={{ width: `${dayProgress}%` }} />
        </div>
      )}

      <div className="fixed top-2 right-2 z-50 flex items-center gap-2">
          <button onClick={handleMobileKeyboardTrigger} className={`w-8 h-8 flex items-center justify-center rounded border transition-colors ${isMobileKeyboardOpen ? 'bg-cyan-600 border-cyan-400 text-white' : (isAscendedBackground ? 'bg-black/50 border-cyan-800 text-cyan-400' : 'bg-slate-900/80 border-slate-700 text-slate-600')}`} title="Open Mobile Keyboard"><Icons.Keyboard /></button>
          {isSandboxUnlocked && showSandboxButton && !isSandboxMode && <button onClick={handleEnterSandbox} className={`w-8 h-8 flex items-center justify-center rounded border transition-colors border-green-500 text-green-500 bg-green-900/20`} title="Enter Sandbox"><Icons.Architect /></button>}
          {unlockedEndings.size > 0 && <button onClick={() => setShowEndings(true)} className={`w-8 h-8 flex items-center justify-center rounded border transition-colors ${isAscendedBackground ? 'bg-black/50 border-cyan-800 text-yellow-400' : 'bg-slate-900/80 border-slate-700 text-yellow-500'}`} title="View Endings"><Icons.Trophy /></button>}
          <button onClick={() => setSoundEnabled(!soundEnabled)} className={`w-8 h-8 flex items-center justify-center rounded border transition-colors ${isAscendedBackground ? 'bg-black/50 border-cyan-800 text-cyan-400' : 'bg-slate-900/80 border-slate-700 text-slate-600'}`} title={soundEnabled ? "Mute Sounds" : "Enable Sounds"}>{soundEnabled ? <Icons.VolumeOn /> : <Icons.VolumeOff />}</button>
          <button onClick={() => setShowChangelog(true)} className={`w-8 h-8 flex items-center justify-center rounded border transition-colors ${isAscendedBackground ? 'bg-black/50 border-cyan-800 text-cyan-400' : 'bg-slate-900/80 border-slate-700 text-slate-400'}`} title="View Changelog"><Icons.Info /></button>
          <div className={`${isAscendedBackground ? 'bg-black/50 border-cyan-800 text-cyan-400' : 'bg-slate-900/80 border-slate-700 text-slate-400'} px-3 py-1 rounded border font-mono text-xs hidden sm:block`}>DAY {day}</div>
          <button onClick={skipDay} disabled={endingState !== 'none' || day === 6} className={`${isAscendedBackground ? 'bg-black/50 border-cyan-800 text-cyan-500 hover:bg-cyan-900/30' : 'bg-slate-800 border-slate-700 text-slate-500 hover:bg-slate-700'} px-2 py-1 rounded text-xs border transition-colors disabled:opacity-30 disabled:cursor-not-allowed`}>SKIP</button>
      </div>

      {isTransitioning && (
          <div className="fixed inset-0 z-[3000] bg-black flex flex-col items-center justify-center p-8 animate-fade-in pointer-events-auto">
              <div className={`font-mono text-xl md:text-2xl animate-pulse text-center ${isAscendedBackground ? 'text-cyan-400 tracking-widest' : 'text-green-500'}`}>{outageText}</div>
          </div>
      )}
      ...
      <div className={`w-full max-w-md lg:max-w-[24rem] xl:max-w-md flex flex-col gap-6 transition-all duration-500 shrink-0 ${day === 3 ? 'translate-y-2 rotate-1 animate-subtle-drift' : ''} ${justTransitioned && day === 3 ? 'animate-glitch' : ''} ${justTransitioned && isAscendedBackground ? 'animate-pulse' : ''} ${isSandboxMode ? 'z-10' : ''}`}>
        <div className="flex flex-col items-center gap-4">
          <Avatar mood={mood} isThinking={isThinking} day={day} absorptionMood={absorptionMood} mousePosition={mousePosition} onClick={handleAvatarClick} />
          
          <div className={`relative p-4 rounded-2xl rounded-tr-none shadow-lg w-full min-h-[5rem] flex items-center justify-center border-2 transition-all duration-500 ${day === 3 ? 'bg-white text-slate-900 border-slate-700 animate-shake' : ''} ${isAscendedBackground ? 'bg-black/60 text-cyan-100 border-cyan-500/30 backdrop-blur-md shadow-[0_0_15px_rgba(6,182,212,0.1)]' : 'bg-white text-slate-900 border-slate-700'} ${mood === Mood.VILE || mood === Mood.PURE_HATRED ? 'border-red-600 bg-red-950/20 text-red-200 shadow-[0_0_20px_red]' : ''}`}>
            ...
            <p className={`text-center font-medium text-lg leading-tight animate-fade-in break-words w-full ${day === 3 ? 'font-mono tracking-widest' : ''} ${isAscendedBackground ? 'font-sans font-light tracking-wide' : ''}`}><Typewriter text={comment} /></p>
          </div>
        </div>

        {endingState === 'decision' && currentDialogueNode && (
             <div className="w-full bg-black/80 border border-red-500/50 p-6 rounded-xl animate-fade-in flex flex-col gap-4 shadow-[0_0_30px_rgba(220,38,38,0.3)] backdrop-blur-sm relative z-30">
                 ...
                 <div className="flex flex-col gap-3">
                     {DIALOGUE_TREE[currentDialogueNode].choices.map((choice, index) => (
                         <button key={index} onClick={() => handleDialogueChoice(choice)} className="p-4 bg-slate-900/80 border border-slate-700 hover:border-cyan-500 hover:bg-cyan-900/20 text-slate-300 hover:text-cyan-300 transition-all text-sm font-mono text-left">{index + 1}. "{choice.text}"</button>
                     ))}
                 </div>
             </div>
        )}
      ...
      {endingState === 'none' && (
          <div className={`w-full max-w-md lg:max-w-[24rem] xl:max-w-md flex flex-col gap-6 shrink-0 ${day === 3 ? '-rotate-1 opacity-90' : ''} ${isSandboxMode ? 'z-10' : ''}`}>
              <div className={`p-6 rounded-2xl relative group transition-colors duration-500 ${isAscendedBackground ? 'bg-slate-900/40 border border-slate-800' : 'bg-slate-900/50 border border-slate-800'}`}>
                   <div className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-800 text-xs text-slate-300 px-2 py-1 rounded opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap border border-slate-600">Regulates the AI's aggression level.</div>
                  <label className="text-slate-400 text-sm font-bold uppercase tracking-wider mb-2 block text-center">Hostility Regulator</label>
                  <div className="flex items-center gap-4 mb-4">
                      <MiniBotCalm /><input type="range" min="0" max="100" value={hostility} onChange={(e) => setHostility(Number(e.target.value))} className="w-full h-3 bg-slate-800 rounded-lg appearance-none cursor-pointer accent-cyan-500 shadow-inner border border-slate-700" /><MiniBotRage />
                  </div>
                  <div className={`text-center font-mono text-sm border-t border-slate-800 pt-2 flex justify-between items-center ${hostility > 80 ? 'text-red-400 animate-pulse' : 'text-slate-300'}`}><span>STATUS: {getHostilityLabel(hostility)}</span><button onClick={handleResetData} className="text-[9px] text-red-900 hover:text-red-500 uppercase font-bold tracking-widest transition-colors">[WIPE DATA]</button></div>
              </div>

               <div className={`rounded-2xl border overflow-hidden flex flex-col transition-all duration-300 ${isAscendedBackground ? 'bg-slate-900/40 border-slate-800' : 'bg-slate-900/50 border-slate-800'}`}>
                  <button onClick={() => setShowHistory(!showHistory)} className="p-3 bg-slate-800/80 border-b border-slate-700 flex justify-between items-center w-full hover:bg-slate-800 transition-colors">
                      <h3 className="text-slate-200 font-bold flex items-center gap-2 text-sm uppercase tracking-wider"><span className="text-xs">💾</span> System Logs</h3>
                      <div className="flex items-center gap-3"><span className="text-[10px] text-slate-500 font-mono">{history.length} ITEMS</span><span className={`text-slate-400 transform transition-transform ${showHistory ? 'rotate-180' : ''}`}>▼</span></div>
                  </button>
                  
                  {showHistory && (
                      <div className="flex flex-col h-[200px]">
                          <div className="flex justify-end p-2 bg-slate-800/30 border-b border-slate-700/50"><button onClick={handleClearHistory} className="text-[10px] text-red-400 hover:text-red-300 px-2 py-1 rounded border border-red-900/50 hover:bg-red-900/20 transition-colors font-mono uppercase">Clear Logs</button></div>
                          <div className="overflow-y-auto p-3 space-y-3 custom-scrollbar flex flex-col-reverse flex-1">
                              {history.map((item) => (
                                    <div key={item.id} className="bg-slate-800/50 p-3 rounded border-l-2 border-slate-600 text-xs font-mono animate-slide-in hover:bg-slate-800 transition-colors">
                                        <div className="flex justify-between text-slate-400 mb-1"><span>{item.expression}</span><span className="text-cyan-500 font-bold">» {item.result}</span></div>
                                        <p className="text-slate-300 text-[10px] leading-tight opacity-80 border-t border-slate-700/50 pt-1 mt-1">// {item.comment}</p>
                                    </div>
                                ))}
                          </div>
                      </div>
              ...
              <div className={`rounded-2xl border flex flex-col transition-all duration-300 ${isAscendedBackground ? 'bg-slate-900/40 border-slate-800' : 'bg-slate-900/50 border-slate-800'}`}>
                    <div className="p-4 border-b border-slate-800 bg-slate-900/30 flex justify-between items-center min-h-[3.5rem]">
                         <div className="flex flex-col">
                             ...
                             {selectedInventoryMood && (<div className={`text-xs font-bold ${MOOD_DETAILS[selectedInventoryMood].color.split(' ')[1]}`}>{MOOD_DETAILS[selectedInventoryMood].title}</div>)}
                         </div>
                         {isSandboxMode && selectedInventoryMood && (
                             <div className="flex gap-2">
                                 {forcedMood === selectedInventoryMood ? (<button onClick={() => { setForcedMood(null); playSfx('click'); }} className="text-[9px] bg-red-900/20 border border-red-500 text-red-400 px-2 py-1 rounded uppercase tracking-wider hover:bg-red-900/40 transition-colors animate-pulse">EJECT</button>) : (<button onClick={() => handleInjectMood(selectedInventoryMood)} className="text-[9px] bg-green-900/20 border border-green-500 text-green-400 px-2 py-1 rounded uppercase tracking-wider hover:bg-green-900/40 transition-colors">INJECT</button>)}
                             </div>
                         )}
                         {forcedMood && (!selectedInventoryMood || forcedMood !== selectedInventoryMood) && (
                             <button onClick={() => { setForcedMood(null); playSfx('click'); }} className="text-[10px] text-red-400 border border-red-500/50 hover:bg-red-900/30 px-2 py-1 rounded uppercase tracking-wider animate-pulse transition-colors flex items-center gap-2" title="Clear Override"><span>OVERRIDE: {MOOD_DETAILS[forcedMood].title}</span><span className="font-bold">×</span></button>
                         )}
                    </div>
                    <div className="p-4 bg-slate-950/50 rounded-b-2xl">
                        <div className="grid grid-cols-6 gap-2">
                            {Object.values(Mood).map(m => (<MoodOrb key={m} mood={m} isDiscovered={discoveredMoods.has(m)} isSelected={selectedInventoryMood === m} isForced={forcedMood === m} size="sm" onClick={() => handleInventoryClick(m)} />))}
                        </div>
                        <div className="mt-3 text-[10px] text-slate-600 font-mono text-center h-4 opacity-70 border-t border-slate-800/50 pt-2">{forcedMood ? `ACTIVE_OVERRIDE_SEQUENCE: ${MOOD_DETAILS[forcedMood].title}` : `MEMORY_INTEGRITY: ${Math.floor((discoveredMoods.size / Object.keys(MOOD_DETAILS).length) * 100)}%`}</div>
                    </div>
              </div>
              <div className="opacity-90"><SystemTerminal logs={systemLogs} /></div>
          </div>
      )}
      ...
      <div className={`w-full max-w-md lg:max-w-[24rem] xl:max-w-md z-10 transition-all duration-500 shrink-0 ${day === 3 ? 'rotate-1' : ''} ${endingState !== 'none' ? 'hidden' : 'block'}`}>
          <div className={`rounded-3xl shadow-2xl overflow-hidden border-4 flex flex-col mb-6 transition-all duration-500 ${day === 2 ? 'shadow-cyan-900/20 bg-slate-800 border-slate-700' : ''} ${day === 3 ? 'shadow-red-900/40 border-slate-600 bg-slate-800' : ''} ${isAscendedBackground ? 'bg-slate-900/80 border-slate-800 shadow-[0_0_30px_rgba(0,0,0,0.5)] ring-1 ring-white/10' : 'bg-slate-800 border-slate-700'} ${forcedMood && !isAscendedBackground ? 'ring-4 ring-cyan-400/60 ring-offset-4 ring-offset-slate-950' : ''}`}>
              <div className={`p-6 text-right font-mono border-b-4 h-36 flex flex-col justify-end relative overflow-hidden transition-all duration-500 ${day === 3 ? 'bg-[#2a2a2a] border-slate-700' : ''} ${isAscendedBackground ? 'bg-black border-slate-800' : 'bg-[#9ea792] border-slate-700 shadow-[inset_0_2px_10px_rgba(0,0,0,0.1)]'}`}>
                  {forcedMood && (<div className="absolute top-0 right-0 bg-cyan-500 text-black text-[10px] font-bold px-2 py-1 animate-pulse z-20 flex items-center gap-1"><span>FORCE:</span><span className="uppercase">{MOOD_DETAILS[forcedMood].title}</span></div>)}
                  ...
                  <div className={`text-lg break-all opacity-70 mb-1 z-10 ${day === 3 ? 'text-green-500 font-bold' : ''} ${day === 4 ? 'text-slate-400 font-light' : 'text-slate-800'} ${day === 5 || day === 6 ? 'text-emerald-500 font-mono text-xs' : ''}`}>{display || (day === 3 ? 'NULL' : (day === 5 || day === 6 ? 'AWAITING INPUT...' : '0'))}</div>
                  <div className={`text-3xl md:text-4xl font-bold tracking-tight break-all z-10 ${day === 3 ? 'text-green-400 font-mono skew-x-12' : ''} ${day === 4 ? 'text-cyan-400 font-sans' : 'text-slate-900'} ${day === 5 || day === 6 ? 'text-emerald-300 font-mono' : ''}`}>{result ? (day >= 4 ? `= ${result}` : `= ${result}`) : ''}</div>
              </div>
          </div>
          <Keypad onInput={handleInput} onDelete={handleDelete} onClear={handleClear} onCalculate={handleCalculate} onDangerHover={handleDangerButtonHover} disabled={isThinking || endingState !== 'none'} day={day} />
      </div>
      ...
      <style>{`
        ...
        .animate-subtle-drift { animation: subtle-drift 10s ease-in-out infinite; }
